...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def test_export_visualization(self, setting, test=0):
        _, test_loader = self._get_data(flag='test')
        if test:
            print('loading model')
            self.model.load_state_dict(torch.load(os.path.join(f'{self.args.save_dir}/checkpoints/' + setting, 'checkpoint.pth'), map_location=self.device))

        folder_path = f'{self.args.save_dir}/test_results_visulization/' + setting + '/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)

        # Get first batch without iterating through the entire loader
        ...
        with torch.no_grad():
            if self.args.use_amp:
                with torch.cuda.amp.autocast():
                    outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, export_image=True)
            else:
                outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, export_image=True)

        y = outputs[0]
        time_image_input = outputs[1]
        time_image_reconstructed = outputs[2]
        vision_image_input = outputs[3]
        vision_image_reconstructed = outputs[4]

        results = {
            'y': y[0].cpu().numpy(),
            'time_image_input': time_image_input[0].cpu().numpy(),
            'time_image_reconstructed': time_image_reconstructed[0].cpu().numpy(),
            'vision_image_input': vision_image_input[0].cpu().numpy(),
            'vision_image_reconstructed': vision_image_reconstructed[0].cpu().numpy(),
        }

        save_file = os.path.join(folder_path, f'result.npz')
        np.savez_compressed(save_file, **results)
        print(f'Visualization data saved to {save_file}')
        return results
